...
public class H264AvcEncoder {

	private final static String TAG = "H264AvcEncoder";
	...
	@SuppressLint("NewApi")
	public H264AvcEncoder(int width, int height, int framerate, int bitrate,String name) {
	...
	public void StartEncoderThread(){
		Thread EncoderThread = new Thread(new Runnable() {
			...
			public void run() {
				...
				while (isRuning) {
					if (MainActivity.getYUVQueue().size() >0){
						input = MainActivity.getYUVQueue().poll();
						...
						try {
							...
							while (outputBufferIndex >= 0) {
								...
								if(TextUtils.isEmpty(m_name)){
									Util.writeFileToSDCard(outData, "H264", m_name, true, false);
								}
								check(outData);
